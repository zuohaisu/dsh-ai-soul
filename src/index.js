...
export async function apply(ctx, rawConfig = {}) {
  ...
  if (!ctx.systemPrompt?.context) throw new TypeError('dsh-ai-soul runtime error: required DSH systemPrompt service is unavailable')
  ...
  ctx.on('ai-soul/state-committed', (payload) => {
    refreshQueue = refreshQueue.then(async () => {
      if (!payload || typeof payload !== 'object' || Array.isArray(payload) || typeof payload.soulId !== 'string') throw new TypeError('dsh-ai-soul state-committed event requires payload.soulId')
      if (payload.soulId !== config.soulId) return { status: 'ignored', soulId: payload.soulId }
...
export { DSH_SOUL_PRESENCE_RUNTIME_ID, DSH_SOUL_PRESENCE_SURFACES, projectDshSoulPresence } from './adapters/soul-presence.js'
...
export { preflightDshSurfaceContinuity, preflightDshSurfaceContinuityDirs } from './surface-continuity-preflight.js'
