...
function relationshipFact(id = 'rel-fact-1', value = { projectId: 'atlas' }) {
...
function state() {
  ...
  value.relationship.state.push({ id: 'legacy-rel-1', statement: 'Long-term collaborators.' })
  value.relationship.state.push(relationshipFact())
...
test('projects detached cognition while keeping legacy relationship state semantically opaque', () => {
  const current = state()
  ...
  assert.deepEqual(input.cognition.relational, [current.relationship.state[1]])
  assert.equal(input.cognition.relational.some((entry) => entry.id === 'legacy-rel-1'), false)
  ...
  input.cognition.relational[0].value.projectId = 'mutated projection'
  input.event.context.text = 'mutated context'
  assert.deepEqual(current, before)
  assert.equal(event.context.text, 'What should we do next?')
})

test('same event changes semantic relational cognition only when governed RelationshipFacts change', () => {
  const first = state()
  const second = structuredClone(first)
  second.relationship.state = [
    { id: 'legacy-rel-2', statement: 'Different free-form wording.' },
    relationshipFact('rel-fact-2', { projectId: 'orion' }),
  ]
...
test('duplicate semantic relationship fact ids fail closed at cognition projection', () => {
  const current = state()
  const duplicate = structuredClone(current.relationship.state[1])
  current.relationship.state.push(duplicate)

  assert.throws(() => createAppraisalInput({ state: current, ...event }), /ambiguous relationship fact id/)
})
